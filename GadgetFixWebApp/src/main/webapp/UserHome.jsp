...
<body>
  ...
  <section class="bg-dark" id="contact">
      <a id="contact-mail" href="mailto:info@gadgetfix.com"><i class="fa-solid fa-envelope"></i> shiksharajput8851@gmail.com</a>
  ...
  %>
  <div >
	<h5 class="bg-primary text-white p-3 text-center mt-2">All Repair Requests</h5>
  <%
  	DAO db=new DAO();
  ...
  %>
  		...
  		<p > 
  		ID: <b><%= gadget.get("id") %></b> 
  		Name: <b><%= gadget.get("name") %></b> 
  		Brand Name: <b><%= gadget.get("brand_name") %></b> 
  		...
  		<img alt="" src="GetGadgetPhoto?photo_no=1&id=<%=gadget.get("id")%>" height="100px">
  		<img alt="" src="GetGadgetPhoto?photo_no=2&id=<%=gadget.get("id")%>" height="100px">
  		</p>
  <%
  	String status=(String)gadget.get("status");
  	if(status.equalsIgnoreCase("WaitingApproval")){
  %>
  		<a class="btn btn-success btn-sm" href="ChangeGadgetStatus?id=<%=gadget.get("id")%>&status=Accept&type=user&email=<%=gadget.get("repair_expert_email") %>">Accept</a>
  		<a class="btn btn-danger btn-sm" href="ChangeGadgetStatus?id=<%=gadget.get("id")%>&status=Decline&type=user&email=<%=gadget.get("repair_expert_email") %>">Decline</a>
  <%		
  	}else if(status.equalsIgnoreCase("pending")){
  %>
  	<a class="btn btn-danger btn-sm" href="DeleteGadget?id=<%=gadget.get("id")%>">Delete Request</a>
  <%		
  	}
  %>			
  		</div>
